...
| 5.3k | **multi-user, multi-platform (android+web)** | `theapp.feature`'s "Orchestrating multiple users on different platforms as part of same test (android-web)" (`@multiuser-android-web @theapp5 @browserstack`) | `AppBL(userPersona, Platform)`, two personas (`I` on android, `You` on web) | `configs/theapp/theapp_browserstack_android_config.properties` | ✅ **Done, and wired into CI** — see below. |
...
- [x] **5.3k (multi-user, multi-platform: android+web) + CI**: `TheAppMultiUserAndroidWebTestNgTest` — direct port of `theapp.feature`'s android-web multi-user scenario, calling `Drivers.createDriverFor(...)` + `AppBL(userPersona, Platform)` per persona (`"I"` on `Platform.android`, `"You"` on `Platform.web`), unchanged from the step-def's own call sequence. This is also the first (and, per explicit scope, only) TestNG-mode job wired into GitHub Actions: `.github/workflows/TestNG_MultiUser_AndroidWeb_CI.yml`, mirroring the existing `BrowserStack_Android_Web_MultiUser_CI.yml` Cucumber-mode job exactly (same composite action, same BrowserStack secrets) but with `FRAMEWORK=testng` via `additional-env` and `TAG=@multiuser-android-web`. One combined job was chosen over two separate web-only/android-only CI jobs since it exercises both platforms in a single BrowserStack run, matching an existing CI pattern with zero new infrastructure.
  - **Bug found and fixed via the first real CI run**: an initial version held each persona's `AppBL` instance and reused it for both of that persona's calls, per an "always chain / reuse instances" rule added afterward — but `AppBL`'s constructor calls `Runner.setCurrentDriverForUser(...)`, setting a single thread-scoped "current persona" pointer that `ScreenRegistry.getScreen()` reads for *every* subsequent screen resolution, regardless of which BL instance's method is called. Constructing persona `You`'s `AppBL` after `I`'s silently repointed "current persona" to `You` for the rest of the test, so `I`'s later `loginAgain()` call (through the held, stale instance) resolved against the **web** screen implementation instead of android. CI caught this immediately: all 4 soft assertions failed with each persona's expected error message swapped with the other's. Fixed by reconstructing `AppBL` immediately before each interaction (matching the original step-def's own behavior exactly), and corrected the chaining rule in `CLAUDE.md`/`ANTIGRAVITY.md`/`SKILL.md`/the migration guide to call out BL-constructor side effects as a real, checked-for exception, not just "reuse when there's nothing to chain onto."
